...
go mod tidy
export GOOS=linux && export GOARCH=amd64 && go build -o bud-engine-backend
...
imageTag=buddy-registry-vpc.tencentcloudcr.com/bud-engine-server/bud-engine-backend:$buildVersion
if [ "$platform" = "m1" ]; then
	echo "---m1 platform"
	echo $imageTag
